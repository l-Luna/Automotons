...
public class ColourVertexConsumer extends FixedColorVertexConsumer{
	...
	public ColourVertexConsumer(VertexConsumer vertexConsumer, Matrix4f modelMatrix, Matrix3f normalMatrix){
		...
		this.normalMatrix = new Matrix3f(normalMatrix);
	...
	public void next(){
		...
		vec.mul(normalMatrix);
		Direction direction = Direction.getFacing(vec.x, vec.y, vec.z);
		Vector4f vector4f = new Vector4f(x, y, z, 1);
		vector4f.mul(modelMatrix);
		vector4f.rotateY(MathHelper.PI);
		vector4f.rotateX(-MathHelper.HALF_PI);
